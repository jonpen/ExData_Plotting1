...
# Script: plot4.R
#
# This script plots the fourth graph in the first project
...
# 2. Generate plot
# 3. Write the plot to the plot4.png file in the current working directory
#
...
# Graph 1
with(dataTimeframe, { 
  plot(Date, Global_active_power, ylab="Global Active Power", type="n")
  lines(Date, Global_active_power)
})

# Graph 2
with(dataTimeframe, { 
  plot(Date, Voltage, ylab="Voltage", xlab="datetime", type="n")
  lines(Date, Voltage)
})
...
with(dataTimeframe, { 
  ...
  legend("topright", bty="n", lty=c(1,1,1), lwd=c(2,2,2), col=c("black","red","blue"), legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))
})

# Graph 4
with(dataTimeframe, { 
  plot(Date, Global_reactive_power, xlab = "datetime", type="n")
  lines(Date, Global_reactive_power,lwd=0.25)
})
